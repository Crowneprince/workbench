...
def load_and_prepare_data(file_path, sheet_name):
    try:
        ...
        df["Total Claims Paid"] = df["Total Claims Paid"].astype(str).str.replace('-', '0').str.replace(' ', '').astype(float)
        df["Underwriter"] = df["Underwriter"].astype(str).str.replace('-', '0').str.replace(' ', '').astype(float)
        df["LegalCostOnly"] = df["LegalCostOnly"].replace('-', '0').astype(str).str.replace(' ', '').astype(int)
        df["Injured"] = df["Injured"].replace('-', '0').astype(str).str.replace(' ', '').astype(int)
        df['ClaimsLoss'] = df['ClaimsLoss'].replace('-', '0').fillna(0).astype(int)
        df["Gross = Total Reserves + Total Claims Paid"] = df["Gross = Total Reserves + Total Claims Paid"].astype(str).str.replace(" ", "").astype(float).fillna(0.0)

        # Renaming and recalculating fields
        df = df.rename(columns={"Gross = Total Reserves + Total Claims Paid": "Gross"})
...
# For local testing, specify the file path and sheet name
if __name__ == '__main__':
    df_test = load_and_prepare_data("Premium_Model_Data.xlsx", "Data")
    if df_test is not None:
        print("DataFrame head:", df_test.head())
    else:
